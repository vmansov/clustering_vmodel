...
class Combined(Force):
    ...
    def energy(self, cells):
        return reduce(add, (s.energy(cells) for s in self.summands))

    def force(self, cells):
        return reduce(add, (s.force(cells) for s in self.summands))
...
class TargetArea(Force):
    ...
    def force(self, cells):
        # print(cells.by_face('K'), cells.by_face('A0'), cells.mesh.area)
