...
def drawnoplt_videos(cells, ax=None, size=None, tocolor= 'color'):
    ...
    facecolors = cells.properties.get(tocolor, None)
...
# Draw the faces
    ...
    # Get the property values for labeling
    if label in cells.properties:
        labels = cells.properties[label]
        # Round non-integer labels to 3 decimal places
        labels = [round(val, 3) if not isinstance(val, int) else val for val in labels]
    else:
        labels = mesh.face_ids  # Default to face IDs if the label property is not found

    # Annotate each face with its ID
    for face_id in mesh.face_ids:
        # Get the vertices of the current face
        face_edges = mesh.boundary(face_id)  # Get the edges forming the boundary of the face
        face_vertices = mesh.vertices[:, face_edges]  # Extract the vertices of the face

        # Calculate the centroid of the face
        centroid = face_vertices.mean(axis=1)

        # Annotate the face with its ID
        ax.text(centroid[0], centroid[1], str(labels[face_id]), color='black', fontsize=8,
                ha='center', va='center', bbox=dict(facecolor=tag_colors[face_id], alpha=0.7, edgecolor='none',pad=0.3))

    ax.set_xticks([])
    ax.set_yticks([])

    size =  2.0*np.max(mesh.vertices[0])
    lim = [-0.55*size, 0.55*size]
    ax.set_xlim(lim)
    ax.set_ylim(lim)
    return fig
